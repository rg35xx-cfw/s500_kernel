...
static int monitor_probe(struct platform_device *_dev)
{
	...
	node = _dev->dev.of_node;
	monitor_type = of_get_property(node, "fix_vbus_reset", NULL);
	if (!monitor_type)
               printk("%s no config fix_vbus_reset.\n", __func__);
       else {
               fix_vbus_reset = be32_to_cpup(monitor_type);
               printk("%s fix_vbus_reset: %d\n", __func__, fix_vbus_reset);
       }
